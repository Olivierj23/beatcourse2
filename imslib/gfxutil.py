...
font_path = str(Path(Path(__file__).parent, 'fonts', 'Inconsolata-SemiBold.ttf'))
LabelBase.register(name='Inconsolata', fn_regular=font_path)

font_path = str(Path(Path(__file__).parent, 'fonts', 'Arial.ttf'))
LabelBase.register(name='Arial', fn_regular=font_path)

def topleft_label(font_size='20sp', monospace=False, color=(1,1,1,1)):
    """
...
class CLabelRect(InstructionGroup):
    ...
    def set_text(self, text):
        ...
        self.cpos = self.cpos  # to recompute rect pos
    ...
    def get_cpos(self):
        """
        Get the center position of the text
        """
        return self._cpos
...
class AnimGroup(InstructionGroup):
    ...
    def on_update(self):
        ...
        kill_list = [o for o in self.objects if not isinstance(o, Color) and o.on_update(dt) == False]
